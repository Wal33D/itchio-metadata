...
export async function fetchItchGameData({ itchApiKey, author, gameTitle, gameUrl }: FetchGameDataParams = {}): Promise<GameMetadata> {
	...
	if (!ITCH_DATA_URI) {
		throw new Error('Required configuration missing: ITCH_DATA_URI is not set in the environment or as a parameter.');
	}
	...
	if (!ITCH_API_KEY) {
		...
		return {
			...
			authors: detailsData.authors.map((author: { name: string; url: string }) => ({ name: author.name, url: author.url })),
		};
	}
